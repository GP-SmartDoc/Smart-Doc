...
summary_module = SummarizationModule(
    retriever=rag,
    model=model
)

slide_generation_module = lambda prompt, document: generate_slides(
    rag,
    prompt,
    document=document
)

visualization_module = lambda state: (
    "Visualization module is under development. Please check back later."
)
